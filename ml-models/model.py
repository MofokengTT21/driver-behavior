...
df_counts = pd.DataFrame({
    'Class': ['Safe', 'Rushed', 'Aggressive'],
    'Correct': [correct_counts[0], correct_counts[1], correct_counts[2]],
    'Total': [total_counts[0], total_counts[1], total_counts[2]]
})
...
torch.onnx.export(model, (dummy_input, h0, c0), "improved_lstm_model.onnx", 
                  input_names=['input', 'h0', 'c0'], output_names=['output'])
print("Model saved to improved_lstm_model.onnx")
...
with open('./scaler/driver_behavior_scaler_params.json', 'w') as f:
